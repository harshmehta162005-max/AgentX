...
function AgentToolsPanel() {
    ...
    return (
        <div className='bg-white p-5 rounded-2xl shadow '>
            <h2 className='font-semibold mb-4 text-gray-700 '>AI Agent</h2>
            <div>
                {AgentTools.map((tool, index) => (
                    <div key={index} className='flex items-center gap-3 mb-4 p-2 rounded-xl cursor-pointer hover:bg-gray-100 ' onClick={() => onAgentToolClick(tool)}>
                        <tool.icon className='p-2 h-8 w-8 ' style={{ backgroundColor: tool.bgColor }} />
